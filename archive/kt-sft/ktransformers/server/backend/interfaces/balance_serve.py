...
class Engine:
    ...
    def __init__(
        ...
        self.model_runner = ModelRunner(
            self.model, self.device, self.args.use_cuda_graph, page_size=args.page_size, block_num=self.block_num
        )
        # @TODO add config
    ...
    def loop(self):
        ...
        while True:
            ...
            if self.batch is not None:
                ...
                print(
                    f"Model execution time (GPU): {self.model_runner.model_time:.3f} ms,"
                    f" {1000/self.model_runner.model_time:.3f} tokens/s"
                )
                # if self.rank == 0:
...
class BalanceServeInterface(BackendInterfaceBase):
    ...
    def __init__(self, args: ConfigArgs = default_args):
        ...
        p = ctx.Process(
            target=run_engine, args=(self.args, self.token_queue, self.broadcast_endpoint, start_event, kvcache_event)
        )
        p.start()
        processes.append(p)
        kvcache_event.wait()
    ...
    async def inference(
        ...
        if query_add.estimated_length < query_add.query_length:
            raise Exception(
                f"query too long: estimated_length={query_add.estimated_length} < query_length={query_add.query_length}"
            )
