...
try:
    ...
    use_npu = False
from ktransformers.server.balance_serve.settings import (
    sched_ext,
    create_sched_settings,
    create_sched_settings_qwen2moe,
    create_sched_settings_qwen3moe,
    create_sched_settings_glm4moe,
    create_sched_settings_smallthinker,
    create_sched_settings_qwen3next,
)
...
class SchedulerServer:
    def __init__(self, settings, main_args):
        ...
        # HMAC key for authenticating ZMQ messages (issue #2042)
        self.hmac_key = getattr(main_args, "hmac_key", None) or generate_hmac_key()

        # 初始化 ZeroMQ 上下文和套接字
        self.context = zmq.Context()
        self.frontend = self.context.socket(zmq.ROUTER)
        print(f"sched zmq rpc server on port {main_args.sched_port}")
        # Bind to localhost only — never expose scheduler RPC to the network
    ...
    def worker_routine(self):
        ...
        while True:
            try:
                # 接收客户端请求
                message = worker.recv()
                data = loads_signed(message, self.hmac_key)
                ...
                if method == "add_query":
                    ...
                    worker.send(dumps_signed(response, self.hmac_key))
                ...
                # HMAC verification failure — log and drop
                print(f"[SECURITY] Rejected unauthenticated RPC message: {e}")
...
class SchedulerClient:
    def __init__(self, sched_port, hmac_key: bytes = None):
    ...
    def send_request(self, method, params=None):
        ...
        self.socket.send(dumps_signed(request, self.hmac_key))
        response = self.socket.recv()
        # print(response)
        response = loads_signed(response, self.hmac_key)
