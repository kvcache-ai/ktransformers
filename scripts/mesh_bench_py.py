#!/usr/bin/env python3
"""MESH benchmark with memory sampling — Python version.
Usage: python3 mesh_bench_py.py <MODEL> <CAP_LIST> <PORT> <CUDA>
Example: python3 mesh_bench_py.py 35b "64 128 192 224" 10004 0,1,2,3
"""
import subprocess, time, json, threading, requests, sys, os

MODEL = sys.argv[1]  # 35b / 397b
CAP_LIST = [int(x) for x in sys.argv[2].split()]
PORT = int(sys.argv[3])
CUDA = sys.argv[4]
...
RESULT_FILE = f"/tmp/mesh_bench_{MODEL}_results.txt"
with open(RESULT_FILE, "w") as f:
    f.write("cap | decode_tok_s | prefill_tok_s | prefill_s | peak_sys_used_gib | peak_gpu_gib\n")
...
def wait_ready(log_file, server_pid, max_wait=600):
...
def bench_prefill():
    """Benchmark prefill with 2048-token prompt."""
...
for cap in CAP_LIST:
    ...
    # Start server
    log_file = f"/tmp/sglang_mesh_{MODEL}_cap{cap}.log"
    ...
    # Write result
    with open(RESULT_FILE, "a") as f:
        f.write(f"{cap} | {dtps:.2f} | {ptps:.2f} | {ptime:.2f} | {peak_mem:.2f} | {peak_gpu:.2f}\n")
