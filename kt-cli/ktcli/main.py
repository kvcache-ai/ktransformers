...
from ktcli import __version__
from ktcli.commands import bench, config, doctor, download, install, quant, run, sft, version
from ktcli.i18n import t, set_lang
...
app.command(name="download", help="Download model weights")(download.download)
...
def check_first_run() -> None:
    ...
    from ktcli.config.settings import DEFAULT_CONFIG_FILE
...
def _show_first_run_setup(settings) -> None:
    ...
    from ktcli.utils.environment import scan_storage_locations, format_size_gb, scan_models_in_location
    ...
    from ktcli.utils.environment import detect_disk_space_gb
