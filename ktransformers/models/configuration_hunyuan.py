...
class HunYuanConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # For compatibility with KTransformers which expects num_experts_per_tok
        # Set it as a real attribute, not just a property
        self.num_experts_per_tok = moe_topk[0] if isinstance(moe_topk, list) else moe_topk
    ...
    def num_experts_per_tok(self):
        """
        Alias for moe_topk to maintain compatibility with KTransformers.
        KTransformers expects num_experts_per_tok (used by Qwen/DeepSeek/Mixtral),
        but Hunyuan uses moe_topk for the same concept.
        """
        # Handle the case where moe_topk might be a list (per-layer configuration)
        if isinstance(self.moe_topk, list):
            # Return the first value or the most common value
            # KTransformers typically expects a single value
            return self.moe_topk[0] if self.moe_topk else 8
