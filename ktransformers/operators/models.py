...
class KHunyuanModel(BaseInjectedModule):
    """
    HunYuan MoE Model with KTransformers optimizations
    
    Transformer decoder consisting of *config.num_hidden_layers* layers.
    Each layer is a [`HunYuanDecoderLayer`] with MoE support.
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        cache_position: Optional[torch.LongTensor] = None,
        per_layer_prefill_intput_threshold: Optional[int] = None,
    ) -> Union[Tuple, MoeModelOutputWithPast]:
        ...
        # Create position embeddings for HunYuan if needed
        position_embeddings = None
    ...
    def load_layer_to(self, layer: HunYuanDecoderLayer, target: InferenceState):
        """Load HunYuan layer to target device/state"""
        assert isinstance(
            layer, HunYuanDecoderLayer
        ), "module should be HunYuanDecoderLayer"
        ...
        # MLP/MoE components
        if isinstance(layer.mlp, HunYuanMoE):
            # HunYuan MoE with mixed architecture
            layer.mlp.gate.set_inference_mode(target)
            layer.mlp.experts.set_inference_mode(target)
            
            # Shared MLP components
            if hasattr(layer.mlp, 'shared_mlp'):
                layer.mlp.shared_mlp.gate_proj.set_inference_mode(target)
                layer.mlp.shared_mlp.up_proj.set_inference_mode(target)
                layer.mlp.shared_mlp.down_proj.set_inference_mode(target)
                layer.mlp.shared_mlp.act_fn.to(device)
        else:
            # Regular MLP
            layer.mlp.gate_proj.set_inference_mode(target)
            layer.mlp.up_proj.set_inference_mode(target)
