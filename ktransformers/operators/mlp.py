...
from ktransformers.models.modeling_hunyuan import HunYuanMLP
...
class KHunYuanMLP(HunYuanMLP, BaseInjectedModule):
    def __init__(self,
                 key: str,
                 gguf_loader: GGUFLoader,
                 config: PretrainedConfig,
    ...
    def forward(self, x, bsz_tensor=None):
        if bsz_tensor is not None:
            # If batch tensor is provided, use it for optimized computation
            gate_proj = self.gate_proj(x, bsz_tensor)
            up_proj = self.up_proj(x, bsz_tensor)
            down_proj = self.down_proj(self.act_fn(gate_proj) * up_proj, bsz_tensor)
            return down_proj
        else:
            return HunYuanMLP.forward(self, x)