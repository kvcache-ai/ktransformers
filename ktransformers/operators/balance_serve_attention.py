...
from ktransformers.models.modeling_llama4 import Llama4TextAttention
...
class KLlama4Attention(BaseInjectedModule, Llama4TextAttention):
    def __init__(self,
                 key: str,
                 gguf_loader : GGUFLoader,
                 config: PretrainedConfig,
    ...
    def forward(self,
        ...
        # TODO: Use temperature tuning from https://arxiv.org/abs/2501.19399) to NoROPE layers
        if self.attn_temperature_tuning and not self.use_rope:
           attn_scales = (
               torch.log(torch.floor((position_ids.float() + 1.0) / self.floor_scale) + 1.0) * self.attn_scale + 1.0
           )
           attn_scales = attn_scales.view(q_len, 1, 1)
