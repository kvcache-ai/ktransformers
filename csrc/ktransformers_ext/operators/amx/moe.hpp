...
template <class T> class AMX_MOE_TP {
private:
  int tp_part_idx;
...
public:
  ...
  void load_weights() {
    ...
    if (config_.gate_projs.size()) {
      ...
      static uint8_t gate_type = 3, mat_split = 1;
      if (config_.load) {
        std::cout << "Loading from " << prefix << std::endl;
        for (int task_id = 0; task_id < config_.expert_num * gate_type * mat_split; task_id++) {
          uint64_t expert_idx = task_id / (gate_type * mat_split);
          uint8_t mat_class = (task_id % (gate_type * mat_split)) / mat_split;
...
// check process, store down matrix to check
        ...
        pool->do_work_stealing_job(
            ...
            [this, nth](int task_id) {
              ...
              gate_bb_[expert_idx]->from_mat((ggml_bf16_t *)config_.gate_proj +
                                             ...
                                             ith, nth);
              // up part
              up_bb_[expert_idx]->from_mat((ggml_bf16_t *)config_.up_proj +
                                               expert_idx * config_.intermediate_size * config_.hidden_size,
                                           ith, nth);
            },
            nullptr);
      ...
      if (config_.save) {
        pool->do_work_stealing_job(
            config_.expert_num * gate_type, nullptr,
            [this](int task_id) {
              uint64_t expert_idx = task_id / gate_type;
              uint8_t mat_class = task_id % gate_type;
              if (mat_class == 0) { // the up matrix
                size_t size = T::BufferB::required_size(config_.intermediate_size, config_.hidden_size);
                size_t scale_size = config_.intermediate_size * sizeof(float);
  ...
  void warm_up() {
    ...
    std::vector<uint64_t> expert_ids(qlen * config_.routed_expert_num);
    std::vector<float> weights(qlen * config_.routed_expert_num);
    for (int i = 0; i < qlen * config_.routed_expert_num; i++) {
      expert_ids[i] = i % config_.expert_num;
      weights[i] = 0.01;
    }
    forward(qlen, config_.routed_expert_num, expert_ids.data(), weights.data(), input.data(), output.data());
  }
...
#define DIRECT_OR_POOL_BY_QLEN(var, fn)                                                                                \
  ...
  void forward_prefill(int qlen, int k, const uint64_t *expert_ids, const float *weights, const void *input,
                       void *output) {
    auto pool = config_.pool->get_subpool(tp_part_idx);
    ...
    pool->do_work_stealing_job(
        ...
        [this, nth, qlen](int task_id2) {
          ...
          int ith = task_id % nth;
          if (do_up) {
            amx::mat_mul<T>(m_local_num_[expert_idx], config_.intermediate_size, config_.hidden_size,
    ...
    pool->do_work_stealing_job(
        ...
        [this, nth, qlen](int task_id) {
          int expert_idx = m_expert_id_map_[task_id / nth];
          int ith = task_id % nth;
          amx::mat_mul<T>(m_local_num_[expert_idx], config_.hidden_size, config_.intermediate_size,
  ...
  void forward_decode(int k, const uint64_t *expert_ids, const float *weights, const void *input, void *output) {
    int qlen = 1;
    auto pool = config_.pool->get_subpool(tp_part_idx);
#ifdef FORWARD_TIME_PROFILE
    auto start_time = std::chrono::high_resolution_clock::now();
    ...
    pool->do_work_stealing_job(
        ...
        [this, nth, qlen](int task_id2) {
          ...
          int ith = task_id % nth;
          if (do_up) {
            amx::mat_mul<T>(qlen, config_.intermediate_size, config_.hidden_size, gate_up_ba_[0].get(),
                            up_bb_[expert_idx].get(), up_bc_[expert_idx].get(), ith, nth);
            up_bc_[expert_idx]->to_mat(qlen, m_local_up_output_ptr_[expert_idx], ith, nth);
          } else {
            amx::mat_mul<T>(qlen, config_.intermediate_size, config_.hidden_size, gate_up_ba_[0].get(),
                            gate_bb_[expert_idx].get(), gate_bc_[expert_idx].get(), ith, nth);
    ...
    pool->do_work_stealing_job(
        ...
        [this, nth, qlen](int task_id) {
          int expert_idx = m_expert_id_map_[task_id / nth];
          int ith = task_id % nth;
          amx::mat_mul<T>(qlen, config_.hidden_size, config_.intermediate_size, down_ba_[expert_idx].get(),
    ...
    auto forward_total_time = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
    // 在函数末尾一次性打印所有阶段的耗时，并附带 max_local_num 和 qlen
    printf("Profiling Results (numa[%d]): activated_expert: %d, q_input: %ld us, "
...
template <typename K> class TP_MOE<AMX_MOE_TP<K>> : public TP_MOE_Common<AMX_MOE_TP<K>> {
public:
