      ...
      if (is_seq) {
        ...
        backend->do_work_stealing_job(
            ...
            [&](int task_id) {
              ...
              bool ok = llamafile_sgemm(1, 1, config_.q_head_num * config_.head_dim,
                                        anchor_.data() + (layer_idx * config_.max_block_num +
                                                          block_table_before_retrieval_[batch_id][block_idx]) *
                                                             config_.anchor_num * config_.q_head_num * config_.head_dim,
                                        config_.q_head_num * config_.head_dim, avg_q_fp16[batch_id].data(),
                                        config_.q_head_num * config_.head_dim,
                                        block_similar_[batch_id].data() + block_id, 1, 0, 1, GGML_TASK_TYPE_COMPUTE,
                                        GGML_TYPE_F16, GGML_TYPE_F16, GGML_TYPE_F32, GGML_PREC_DEFAULT);
              if (!ok) {
                printf("llamafile_sgemm failed\n");
              }
            },
...
void KVCache::attn_with_kvcache_one_block_(int head_dim, int bsz,
  ...
  if (q_type == GGML_TYPE_F16) {
    ...
    if (rotary_angle != nullptr) {
      ...
      bool ok = llamafile_sgemm(past_kv_len, bsz, head_dim, (ggml_fp16_t *)k_cache, head_dim, (ggml_fp16_t *)q,
                                head_dim, attn_score, past_kv_len, 0, 1, GGML_TASK_TYPE_COMPUTE, k_type, GGML_TYPE_F16,
                                GGML_TYPE_F32, GGML_PREC_DEFAULT);
    ...
    if (rotary_angle != nullptr) {
      ...
      llamafile_sgemm(past_kv_len, bsz, head_dim / 32, (block_q4_0 *)k_cache_with_rope_q4, head_dim / 32,
                      (block_q8_0 *)q, head_dim / 32, attn_score, past_kv_len, 0, 1, GGML_TASK_TYPE_COMPUTE, k_type,
