...
static const bool DISABLE_DYNAMIC_SCHEDULING = []() { return std::getenv("DISABLE_DYNAMIC_SCHEDULING") != nullptr; }();
...
void InNumaPool::process_tasks(int thread_id) {
  ...
  if (DISABLE_DYNAMIC_SCHEDULING) {
    int base = end_ / worker_count;
    int rem = end_ % worker_count;
    int start_id = thread_id * base + std::min(thread_id, rem);
    int end_id = start_id + base + (thread_id < rem ? 1 : 0);
...
void WorkerPool::init(WorkerPoolConfig config) {
  ...
  for (int i = 0; i < config.subpool_count; i++) {
    ...
    std::thread([this, i, this_numa, this_thread_count, this_thread_id_start]() {
      // This is for breakdown analysis
      if (std::getenv("DISABLE_NUMA_AWARE") != nullptr) {
