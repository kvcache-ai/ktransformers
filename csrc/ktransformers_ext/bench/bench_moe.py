...
def bench_moe(quant_mode: str):
    ...
    with torch.inference_mode():
        ...
        # 专家路由索引与权重，每层一个
        gen_iter = 1000
        expert_ids = torch.rand(gen_iter * qlen , expert_num, device="cuda").argsort(dim=-1)[:, :n_routed_experts].reshape(gen_iter, qlen * n_routed_experts).to("cpu").contiguous()
        weights = torch.rand((gen_iter, qlen, n_routed_experts), dtype=torch.float32, device="cuda").to("cpu").contiguous() 
        ...
        bandwidth = hidden_size * intermediate_size * 3 * n_routed_experts * (1/8 * 256 * (1-(31/32)**qlen)) * bytes_per_elem * test_iter / total_time / 1e9  # 单位：GB/s
