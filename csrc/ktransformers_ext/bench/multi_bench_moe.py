...
all_params = {
    # 固定参数
    "test_operator_type": "amx",   # "llamafile" 或 "amx"
    "expert_num": 256,
    "n_routed_experts": 8,
    "hidden_size": 7168,
    ...
    "m_block": 1,             # llamafile 专用
    # "qlen": list(range(1,10)) + list(range(10,100,10)),  # 可选值，llamafile 和 amx 都支持
    # "qlen": [2**i for i in range(11)] * 32,  # 可选值，llamafile 和 amx 都支持
    "qlen": [32*i for i in range(1, 9)],  # 可选值，llamafile 和 amx 都支持
    "layer_num": 2,
    "warm_up_iter": 100,
    "test_iter": 1000,
...
def update_bench_parameters(params):
    ...
    bench.n_routed_experts = params["n_routed_experts"]
