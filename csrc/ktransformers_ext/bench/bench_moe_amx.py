...
def bench_moe(quant_mode: str):
    with torch.inference_mode():
        ...
        for layer_index in range(layer_num):
            ...
            moes.append(moe)
        gen_iter = 1000
        expert_ids = torch.rand(gen_iter * qlen , expert_num, device="cpu").argsort(dim=-1)[:, :n_routed_experts].reshape(gen_iter, qlen * n_routed_experts).to("cpu").contiguous()
        weights = torch.rand((gen_iter, qlen, n_routed_experts), dtype=torch.float32, device="cpu").to("cpu").contiguous()
        ...
        bandwidth = hidden_size * intermediate_size * 3 * expert_num * (1 - (1 - n_routed_experts / expert_num) ** qlen) * bytes_per_elem * test_iter / total_time / 1e9  # 单位：GB/s
        flops = hidden_size * intermediate_size * qlen * 3 * n_routed_experts * 2 * test_iter / total_time / 1e12  # 单位：TFLOPS
