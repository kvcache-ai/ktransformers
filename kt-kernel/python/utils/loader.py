...
class CompressedSafeTensorLoader(SafeTensorLoader):
    ...
    @staticmethod
    def _normalize_rawint4_weight(weight_tensor, key: str = "weight_packed"):
        """Return byte-packed uint8 RAWINT4 weights expected by kt_kernel_ext."""
        if weight_tensor.dim() != 2:
            raise ValueError(f"{key} must be a 2D tensor, got shape {tuple(weight_tensor.shape)}")
    ...
    def load_experts(self, base_key: str, device: str = "cpu"):
        ...
        def load_projection(proj_name: str):
            ...
            for exp_id in range(expert_idx):
                ...
                weight_tensor = self._normalize_rawint4_weight(weight_tensor, weight_key)
