...
def _create_inference_wrapper(
    ...
    # `swiglu_limit != 0` is meaningful only on native MXFP4/MXFP8 or the
    # LLAMAFILE backend. LLAMAFILE validates the GGUF tensor types while loading
    # and applies the clamp in its scalar/NEON-independent activation step.
    extra_kwargs = {}
    if method in ("MXFP4", "MXFP8", "LLAMAFILE"):
