...
def is_amx_weights(model_path):
    """
    判断是否是 AMX 权重，以及 NUMA 数量

    返回:
        (is_amx, numa_count):
            - is_amx: bool, 是否是 AMX 权重
            - numa_count: int, NUMA 数量（如果不是 AMX 则返回 0）
    """
    import re
    from pathlib import Path
    ...
    # 检查前3个文件
    for file_path in safetensors_files[: min(3, len(safetensors_files))]:
        try:
            with safe_open(file_path, framework="pt", device="cpu") as f:
                keys = list(f.keys())

                file_numa_keys = [k for k in keys if ".numa." in k]

                if file_numa_keys:
                    has_numa = True

                    for key in file_numa_keys:
...
def show_model_list() -> None:
    """Display user-registered models."""
    ...
    table.add_column(t("model_column_format"), justify="center")
    ...
    for model in models:
        # Format repo info
        if model.repo_id:
            repo_display = f"{model.repo_type}: {model.repo_id}"
            if len(repo_display) > 30:
                repo_display = repo_display[:27] + "..."
        else:
        ...
        sha256_display = status_map.get(model.sha256_status, model.sha256_status)

        table.add_row(model.name, model.format, repo_display, sha256_display)
    ...
    console.print(f"  • {t('model_usage_scan')}   [cyan]kt model scan[/cyan]")
    console.print(f"  • {t('model_usage_add')}    [cyan]kt model add <path>[/cyan]")
    console.print(f"  • {t('model_usage_info')}   [cyan]kt model info <name>[/cyan]")
    console.print(f"  • {t('model_usage_verbose')} [cyan]kt model list --verbose[/cyan]")
    console.print()
...
[bold]Status:[/bold] {path_status}"""
        ...
        def format_model_row(model, moe_info=None, numa_count=None):
            ...
            if numa_count is not None:
                ...
                single_expert_display = (
                    f"[green]{format_size(int(moe_info['single_expert_size_gb'] * 1024**3))}[/green]"
                )
                skeleton_display = f"[blue]{format_size(int(moe_info['rest_size_gb'] * 1024**3))}[/blue]"
                row.extend([experts_display, single_expert_display, skeleton_display])
        ...
        if amx_models:
            ...
            for model, numa_count in amx_models:
                row = format_model_row(model, numa_count=numa_count)
                table.add_row(*row)
        ...
        if gpu_models:
            ...
            if show_moe and analyze_moe_model:
                table.add_column("Exps", justify="center")
                table.add_column("1-Expert", justify="right")
                table.add_column("Skeleton", justify="right")
            ...
            for model in gpu_models:
                ...
                row = format_model_row(model, moe_info=moe_info)
                table.add_row(*row)
                ...
                if model.id in attached_cpu_models:
                    ...
                    num_cols = len(row)
                    sub_row = [f"  [dim]↳ CPU: {cpu_names}[/dim]"] + [""] * (num_cols - 1)
                    table.add_row(*sub_row, style="dim")

            console.print(table)
            console.print()

        # Table 4: Failed MoE Analysis (if any)
        if moe_failed_models:
            ...
            for model, error in moe_failed_models:
                from kt_kernel.cli.utils.model_scanner import format_size
                ...
                table.add_row(model.name, model.path, size_display, error)
...
# NOTE: search command disabled - no longer using builtin model registry
# Users can use 'kt model list' instead
...
def scan(
    auto_add: bool = typer.Option(False, "--auto-add", help="Automatically add all found models without prompting"),
    min_size: float = typer.Option(10.0, "--min-size", help="Minimum folder size in GB (default: 10.0)"),
) -> None:
    """Scan configured paths for models."""
    from rich.table import Table
    from rich.prompt import Prompt
    from kt_kernel.cli.utils.user_model_registry import UserModelRegistry, UserModel
    from kt_kernel.cli.utils.model_scanner import scan_directory, format_size
    from kt_kernel.cli.config.settings import get_settings
    ...
    if not model_paths:
        print_error(t("model_no_storage_paths"))
        console.print()
        console.print(f"  {t('model_add_path_hint')} [cyan]kt model path-add <path>[/cyan]")
        console.print()
        raise typer.Exit(1)

    console.print()
    print_info(t("model_scanning_paths"))
    for path in model_paths:
        console.print(f"  • {path}")
    console.print()

    # Get already registered paths to exclude
    existing_models = registry.list_models()
    exclude_paths = {model.path for model in existing_models}

    # Scan all paths
    all_scanned = []
    all_warnings = []

    with console.status(t("model_scanning_progress")):
        for model_path in model_paths:
            if not model_path.exists():
                print_warning(t("model_path_not_exist", path=str(model_path)))
                continue

            try:
                scanned, warnings = scan_directory(model_path, min_size_gb=min_size, exclude_paths=exclude_paths)
                all_scanned.extend(scanned)
                all_warnings.extend(warnings)
            except Exception as e:
                print_error(t("model_scan_error", path=str(model_path), error=str(e)))

    # Show warnings
    if all_warnings:
        console.print()
        print_warning(t("model_scan_warnings_title"))
    ...
    if not all_scanned:
        print_warning(t("model_scan_no_models_found"))
        console.print()
        console.print(f"  {t('model_scan_check_paths_hint')}")
        console.print(f"  {t('model_scan_min_size_hint', size=min_size)}")
        console.print()
        return

    # Helper function to display the models table
    def display_models_table(scanned_models, custom_names_dict, custom_repos_dict, skip_set):
        """Display a table of scanned models with current status."""
        table = Table(show_header=True, header_style="bold")
        table.add_column("#", justify="right", style="dim")
        table.add_column(t("model_column_name"), style="cyan")
        table.add_column(t("model_column_format"), justify="center")
        table.add_column(t("model_column_size"), justify="right")
        table.add_column(t("model_column_repo"), style="yellow")

        for i, scanned in enumerate(scanned_models):
            # Get current name (custom or default)
            name = custom_names_dict.get(i, scanned.folder_name)

            # Get repo info if configured
            repo_info = ""
            if i in custom_repos_dict:
                repo_type, repo_id = custom_repos_dict[i]
                # Abbreviate repo type for display
                repo_abbr = "hf" if repo_type == "huggingface" else "ms"
                repo_info = f"[yellow]{repo_abbr}[/yellow]:[dim]{repo_id}[/dim]"
            else:
                repo_info = "[dim]-[/dim]"
            ...
            if i in skip_set:
                ...
                repo_info = f"[dim strikethrough]{repo_info}[/dim strikethrough]"

            table.add_row(
                str(i + 1),
                name,
                scanned.format,
                f"{format_size(scanned.size_bytes)} ({scanned.file_count} files)",
                repo_info,
            )
    ...
    console.print(f"[bold green]{t('model_scan_found_title', count=len(all_scanned))}[/bold green]\n")
    display_models_table(all_scanned, {}, {}, set())
    console.print()

    # Interactive mode or auto-add
    if auto_add:
        # Auto-add all models
        print_info(t("model_scan_auto_adding"))
        added_count = 0

        for scanned in all_scanned:
            model_name = scanned.folder_name
            # Check for name conflicts
            if registry.check_name_conflict(model_name):
                model_name = registry.suggest_name(model_name)

            user_model = UserModel(
                name=model_name, path=scanned.path, format=scanned.format, sha256_status="not_checked"
            )

            try:
                registry.add_model(user_model)
                print_success(t("model_added", name=model_name))
                added_count += 1
            except Exception as e:
                print_error(t("model_add_failed", name=model_name, error=str(e)))

        console.print()
        print_success(t("model_scan_complete", count=added_count))
        console.print()
    else:
        # Interactive mode
        console.print(f"[bold]{t('model_scan_interactive_prompt')}[/bold]")
        console.print(f"  • [cyan]edit <id>[/cyan] - {t('model_scan_cmd_edit')}")
        console.print(f"  • [cyan]del <id>[/cyan]  - {t('model_scan_cmd_delete')}")
        console.print(f"  • [cyan]done[/cyan]      - {t('model_scan_cmd_done')}")
        console.print()

        # Track which models to skip, custom names and repo info
        to_skip = set()
        custom_names = {}  # idx -> custom_name
        custom_repos = {}  # idx -> (repo_type, repo_id)

        while True:
            cmd = Prompt.ask(">", default="done").strip().lower()
            ...
            if cmd == "done":
                ...
                try:
                    ...
                    if 0 <= idx < len(all_scanned):
                        to_skip.add(idx)
                        print_success(t("model_scan_marked_skip", id=idx + 1))
                        console.print()
                        # Redisplay table to show updated status
                        display_models_table(all_scanned, custom_names, custom_repos, to_skip)
                    else:
                        print_error(t("model_scan_invalid_id", id=cmd.split()[1]))
                except (ValueError, IndexError):
                    print_error(t("model_scan_invalid_command"))
            elif cmd.startswith("edit "):
                try:
                    idx = int(cmd.split()[1]) - 1
                    if 0 <= idx < len(all_scanned):
                        console.print()
                        scanned = all_scanned[idx]
                        console.print(f"[bold]Model #{idx+1}:[/bold] {scanned.path}")
                        console.print(f"Default name: [cyan]{scanned.folder_name}[/cyan]")
                        console.print()

                        # Let user set custom name
                        new_name = Prompt.ask(
                            t("model_add_prompt_name"), default=custom_names.get(idx, scanned.folder_name)
                        ).strip()

                        if new_name:
                            # Check for conflicts with existing models and other custom names
                            conflict = False
                            if registry.check_name_conflict(new_name):
                                print_warning(t("model_add_name_conflict", name=new_name))
                                conflict = True

                            # Check conflicts with other custom names in this session
                            for other_idx, other_name in custom_names.items():
                                if other_name == new_name and other_idx != idx:
                                    print_warning(t("model_add_name_conflict", name=new_name))
                                    conflict = True
                                    break

                            if not conflict:
                                custom_names[idx] = new_name
                                print_success(f"Name: [cyan]{new_name}[/cyan]")

                        console.print()
                        # Ask about repository configuration
                        configure_repo = Prompt.ask(t("model_add_configure_repo"), choices=["y", "n"], default="n")

                        if configure_repo.lower() == "y":
                            repo_type = Prompt.ask(
                                t("model_add_repo_type_prompt"),
                                choices=["huggingface", "modelscope", "cancel"],
                                default="huggingface",
                            )

                            if repo_type != "cancel":
                                repo_id = Prompt.ask(t("model_add_repo_id_prompt")).strip()

                                if repo_id:
                                    custom_repos[idx] = (repo_type, repo_id)
                                    print_success(f"Repository: [cyan]{repo_type}[/cyan] → [cyan]{repo_id}[/cyan]")
                                else:
                                    # Remove repo if empty
                                    custom_repos.pop(idx, None)
        ...
        # Add models
        console.print()
        print_info(t("model_scan_adding_models", count=len(all_scanned) - len(to_skip)))
        added_count = 0

        for i, scanned in enumerate(all_scanned):
            if i in to_skip:
                continue

            # Use custom name if set, otherwise use folder name
            model_name = custom_names.get(i, scanned.folder_name)

            # Check for name conflicts (only if not already set as custom name)
            if i not in custom_names and registry.check_name_conflict(model_name):
                model_name = registry.suggest_name(model_name)

            # Get repo info if configured
            repo_type = None
            repo_id = None
            if i in custom_repos:
                repo_type, repo_id = custom_repos[i]
        ...
        if added_count > 0:
            ...
            print_step("Auto-detecting repository information from README.md files...")
            console.print()

            from kt_kernel.cli.utils.repo_detector import scan_models_for_repo, apply_detection_results
            from rich.table import Table

            # Get newly added models (without repo_id)
            all_models = registry.list_models()
            models_without_repo = [m for m in all_models if not m.repo_id]

            if models_without_repo:
                results = scan_models_for_repo(models_without_repo)

                if results["detected"]:
                    console.print("[bold green]✓ Repository Information Detected[/bold green]")
                    console.print()
                    ...
                    # Auto-apply detected repo info
                    updated_count = apply_detection_results(results, registry)

                    if updated_count > 0:
                        print_success(f"✓ Auto-configured repository info for {updated_count} model(s)")
                        console.print()

            console.print(f"  {t('model_scan_next_steps')}")
            console.print(f"  • {t('model_scan_view_hint')} [cyan]kt model list[/cyan]")
            console.print(f"  • {t('model_scan_edit_hint')} [cyan]kt model edit <name>[/cyan]")
            console.print(f"  • Verify models: [cyan]kt model verify <name>[/cyan]")
            console.print()
        else:
            print_warning(t("model_scan_no_models_added"))
            console.print()


@app.command(name="add")
def add_model(
    path: str = typer.Argument(..., help="Path to model directory"),
) -> None:
    """Add a single model to the registry."""
    from pathlib import Path
    from rich.prompt import Prompt, Confirm
    from kt_kernel.cli.utils.user_model_registry import UserModelRegistry, UserModel
    from kt_kernel.cli.utils.model_scanner import scan_single_path, format_size
    ...
    if not path_obj.exists():
        print_error(t("model_add_path_not_exist", path=str(path_obj)))
        raise typer.Exit(1)

    if not path_obj.is_dir():
        print_error(t("model_add_not_directory", path=str(path_obj)))
        raise typer.Exit(1)

    # Check if already registered
    existing = registry.find_by_path(str(path_obj))
    if existing:
        print_warning(t("model_add_already_registered", name=existing.name))
        console.print()
        console.print(f"  {t('model_add_view_hint')} [cyan]kt model info {existing.name}[/cyan]")
        console.print()
        raise typer.Exit(1)

    # Scan the path
    console.print()
    with console.status(t("model_add_scanning")):
        try:
            scanned = scan_single_path(path_obj)
        except ValueError as e:
            print_error(str(e))
            raise typer.Exit(1)
        except Exception as e:
            print_error(t("model_add_scan_failed", error=str(e)))
            raise typer.Exit(1)

    if not scanned:
        print_error(t("model_add_no_model_files"))
        console.print()
        console.print(f"  {t('model_add_supported_formats')}")
        console.print()
        raise typer.Exit(1)

    # Display detected info
    print_success(
        t("model_add_detected", format=scanned.format, count=scanned.file_count, size=format_size(scanned.size_bytes))
    )
    console.print()

    # Prompt for name
    default_name = scanned.folder_name
    if registry.check_name_conflict(default_name):
        suggested_name = registry.suggest_name(default_name)
        console.print(f"[yellow]{t('model_add_name_conflict', name=default_name)}[/yellow]")
        default_name = suggested_name

    model_name = Prompt.ask(t("model_add_prompt_name"), default=default_name)

    # Check name conflict again
    if registry.check_name_conflict(model_name):
        print_error(t("model_add_name_exists", name=model_name))
        raise typer.Exit(1)

    # Ask about repo configuration
    console.print()
    configure_repo = Confirm.ask(t("model_add_configure_repo"), default=False)
    ...
    if configure_repo:
        # Choose repo type
        console.print()
        console.print(t("model_add_repo_type_prompt"))
        console.print("  [1] HuggingFace")
        console.print("  [2] ModelScope")
        console.print("  [3] Skip")

        choice = Prompt.ask(t("model_add_choice"), choices=["1", "2", "3"], default="3")

        if choice == "1":
            repo_type = "huggingface"
            repo_id = Prompt.ask(t("model_add_repo_id_prompt", example="deepseek-ai/DeepSeek-V3"))
        elif choice == "2":
            repo_type = "modelscope"
            repo_id = Prompt.ask(t("model_add_repo_id_prompt", example="deepseek/DeepSeek-V3"))
    ...
    user_model = UserModel(
        ...
        repo_type=repo_type,
        repo_id=repo_id,
        sha256_status="no_repo" if not repo_id else "not_checked",
    )

    try:
        registry.add_model(user_model)
        console.print()
        print_success(t("model_add_success", name=model_name))
        console.print()

        if repo_id:
            console.print(f"  {t('model_add_verify_hint')} [cyan]kt model verify {model_name}[/cyan]")
        else:
            console.print(f"  {t('model_add_edit_later_hint')} [cyan]kt model edit {model_name}[/cyan]")
        console.print()

    except Exception as e:
        print_error(t("model_add_failed_generic", error=str(e)))
        raise typer.Exit(1)


@app.command(name="edit")
def edit_model(
    name: str = typer.Argument(..., help="Name of model to edit"),
) -> None:
    ...
    while True:
        ...
        # Format repo info
        if model.repo_id:
            repo_info = f"{model.repo_type}: {model.repo_id}"
        else:
            repo_info = "-"
        ...
        if is_cpu_model and model.gpu_model_ids:
            ...
            gpu_links_info = f"\n[bold]GPU Links:[/bold]  {', '.join(gpu_names)}"
...
[bold]SHA256:[/bold]     {sha256_display}{gpu_links_info}"""
        ...
        if is_cpu_model:
            console.print("  [4] Manage GPU Links")
            console.print("  [5] " + t("model_edit_option_cancel"))
            console.print()
            choice = Prompt.ask(t("model_edit_choice_prompt"), choices=["1", "2", "3", "4", "5"], default="5")
        else:
            console.print("  [4] " + t("model_edit_option_cancel"))
            console.print()
            choice = Prompt.ask(t("model_edit_choice_prompt"), choices=["1", "2", "3", "4"], default="4")

        if choice == "1":
            # Edit name
            console.print()
            new_name = Prompt.ask(t("model_edit_new_name"), default=model.name)

            if new_name != model.name:
                # Check for conflict
                if registry.check_name_conflict(new_name, exclude_name=model.name):
                    print_error(t("model_edit_name_conflict", name=new_name))
                    continue

                # Update
                old_name = model.name
                registry.update_model(old_name, {"name": new_name})
                model.name = new_name  # Update local copy
                print_success(t("model_edit_name_updated", old_name=old_name, new_name=new_name))
            ...
            if repo_choice == "3":
                ...
                registry.update_model(model.name, updates)
                model.repo_type = None
                model.repo_id = None
                model.sha256_status = "no_repo"
                print_success(t("model_edit_repo_removed"))
            else:
                # Set repo
                ...
                current_default = model.repo_id if model.repo_id and model.repo_type == repo_type else ""
                repo_id = Prompt.ask(
                    t("model_edit_repo_id_prompt", example=example),
                    default=current_default if current_default else None,
                )

                updates = {"repo_type": repo_type, "repo_id": repo_id, "sha256_status": "not_checked"}
                registry.update_model(model.name, updates)
                model.repo_type = repo_type
                model.repo_id = repo_id
                model.sha256_status = "not_checked"
                print_success(t("model_edit_repo_updated"))
            ...
            # Manage GPU Links (only for CPU models)
            console.print()
            console.print(f"[bold cyan]Manage GPU Links for {model.name}[/bold cyan]")
            console.print()

            # Show current links
            if model.gpu_model_ids:
                console.print("[bold]Current GPU links:[/bold]")
                for i, gpu_id in enumerate(model.gpu_model_ids, 1):
                    gpu_obj = registry.get_model_by_id(gpu_id)
                    if gpu_obj:
                ...
                console.print("[dim]No GPU links configured.[/dim]")
                console.print()

            console.print("Options:")
            console.print("  [1] Add GPU link")
            console.print("  [2] Remove GPU link")
            console.print("  [3] Clear all GPU links")
            console.print("  [4] Back to main menu")
            console.print()

            link_choice = Prompt.ask("Choose option", choices=["1", "2", "3", "4"], default="4")
            ...
            if link_choice == "1":
                ...
                if not available_gpu_models:
                    console.print()
                    console.print("[yellow]No GPU models available to link.[/yellow]")
                    console.print()
                else:
                    console.print()
                    console.print("Available GPU models:")
                    for i, gpu_m in enumerate(available_gpu_models, 1):
                        already_linked = model.gpu_model_ids and gpu_m.id in model.gpu_model_ids
                        status = " [dim](already linked)[/dim]" if already_linked else ""
                        console.print(f"  [{i}] {gpu_m.name}{status}")
                    console.print()

                    gpu_choice = Prompt.ask("Enter GPU model number to add", default="0")
                    try:
                        gpu_idx = int(gpu_choice) - 1
                        if 0 <= gpu_idx < len(available_gpu_models):
                            selected_gpu = available_gpu_models[gpu_idx]

                            # Add to gpu_model_ids
                            current_ids = list(model.gpu_model_ids) if model.gpu_model_ids else []
                            if selected_gpu.id not in current_ids:
                                current_ids.append(selected_gpu.id)
                                registry.update_model(model.name, {"gpu_model_ids": current_ids})
                                model.gpu_model_ids = current_ids
                                console.print()
                                print_success(f"Added GPU link: {selected_gpu.name}")
                            else:
                                console.print()
                                console.print("[yellow]This GPU model is already linked.[/yellow]")
                        else:
                            console.print()
                ...
                if not model.gpu_model_ids:
                    console.print()
                    console.print("[yellow]No GPU links to remove.[/yellow]")
                    console.print()
                else:
                    console.print()
                    console.print("Choose GPU link to remove:")
                    gpu_list = []
                    for i, gpu_id in enumerate(model.gpu_model_ids, 1):
                        gpu_obj = registry.get_model_by_id(gpu_id)
                    ...
                    remove_choice = Prompt.ask("Enter number to remove", default="0")
                    try:
                        remove_idx = int(remove_choice) - 1
                        if 0 <= remove_idx < len(gpu_list):
                            removed_id, removed_name = gpu_list[remove_idx]
                            new_ids = [gid for gid in model.gpu_model_ids if gid != removed_id]
                            registry.update_model(model.name, {"gpu_model_ids": new_ids if new_ids else None})
                            model.gpu_model_ids = new_ids if new_ids else None
                            console.print()
                            print_success(f"Removed GPU link: {removed_name}")
                        else:
                            console.print()
                            console.print("[red]Invalid choice.[/red]")
                    except ValueError:
                ...
                if not model.gpu_model_ids:
                    console.print()
                    console.print("[yellow]No GPU links to clear.[/yellow]")
                    console.print()
                else:
                    if Confirm.ask("Remove all GPU links?", default=False):
                        registry.update_model(model.name, {"gpu_model_ids": None})
                        model.gpu_model_ids = None
                        console.print()
                        print_success("All GPU links removed.")
                    else:
                        console.print()
                        print_info("Cancelled.")

        elif choice == "4" or (choice == "5" and is_cpu_model):
            # Cancel
            console.print()
...
def verify_model(
    ...
    def show_model_table():
        all_models = registry.list_models()
        verifiable_models = [m for m in all_models if m.repo_id]
        ...
        console.print()
        console.print("[bold]Select a model to verify:[/bold]\n")
        ...
        for i, model in enumerate(verifiable_models, 1):
            repo_abbr = "hf" if model.repo_type == "huggingface" else "ms"
            repo_display = f"{repo_abbr}:{model.repo_id}"
            ...
            table.add_row(str(i), model.name, model.format, repo_display, status_icon)
    ...
    while True:
        ...
        # Use progress bar for verification
        result = None
        failed_files = []
        ...
        with Progress(
            ...
            def progress_callback(msg: str, total=None, current=None):
                nonlocal task_id
                ...
                if "Using" in msg and "workers" in msg:
                    ...
                    if task_id is None:
                        task_id = progress.add_task("[cyan]Fetching remote hashes...", total=None)
                elif "✓ Fetched" in msg:
                ...
                elif "Calculating SHA256" in msg:
                    if task_id is not None:
                        progress.remove_task(task_id)
                    task_id = progress.add_task("[yellow]Hashing local files...", total=total or 100)
                elif "✓ Calculated" in msg:
                    if task_id is not None:
                        progress.update(task_id, completed=True)
                        progress.remove_task(task_id)
                        task_id = None
                    console.print(f"  [green]{msg}[/green]")
                elif "Comparing" in msg:
                    if task_id is not None:
                        progress.remove_task(task_id)
                    task_id = progress.add_task("[blue]Comparing hashes...", total=total or 100)
                elif "[" in msg and "/" in msg and "]" in msg:
                    # Progress update: [1/291] ✓ filename
                    if verbose:
                        # In verbose mode, print full output line by line
                        if task_id is not None:
                            progress.remove_task(task_id)
                            task_id = None
                        if "✓" in msg:
                            ...
                            failed_files.append(parts)
                    else:
                        # In non-verbose mode, use progress bar
                        if "✓" in msg:
                            current_file["name"] = msg.split("✓")[1].strip().split("(")[0].strip()
                            if task_id is not None:
                                progress.update(
                                    task_id, advance=1, description=f"[blue]Verifying: {current_file['name'][:40]}..."
                                )
                        elif "✗" in msg:
                            # Track failed files
            ...
            result = verify_model_integrity_with_progress(
                repo_type=selected_model.repo_type,
                repo_id=selected_model.repo_id,
                local_dir=Path(selected_model.path),
        ...
        if result["status"] == "passed":
            ...
            if Confirm.ask("[yellow]Do you want to repair (re-download) the failed files?[/yellow]", default=True):
                ...
                for f in result["files_failed"]:
                    if "(missing)" in f:
                        clean_name = f.replace(" (missing)", "").strip()
                    elif "(hash mismatch)" in f:
                        clean_name = f.replace(" (hash mismatch)", "").strip()
                    else:
                        clean_name = f.strip()
                    files_to_download.append(clean_name)
