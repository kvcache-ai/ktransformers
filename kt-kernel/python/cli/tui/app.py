...
class ModelManagerApp(App):
    ...
    def load_models(self) -> None:
        ...
        try:
            ...
            # Check if no models found and prompt for scan
            if total == 0 and not hasattr(self, "_initial_scan_prompted"):
                self._initial_scan_prompted = True
                self._prompt_initial_scan()
    ...
    def _prompt_initial_scan(self) -> None:
        """Prompt user to scan for models on first launch"""
        from kt_kernel.cli.config.settings import get_settings
        ...
        if not model_paths or not any(p.exists() for p in model_paths):
            # No paths configured or paths don't exist
            self.push_screen(
                ConfirmDialog(
                    "No Models Found",
                    "No models found in registry.\n\n"
                    "Would you like to configure model paths and scan?\n"
                    "You can add paths in Settings (press 's' or '7').",
                    confirm_variant="primary",
                ),
                callback=self._on_initial_scan_prompt,
            )
        else:
            # Paths exist but no models found, offer to scan
            paths_str = "\n".join(f"  • {p}" for p in model_paths)
            self.push_screen(
                ConfirmDialog(
                    "No Models Found",
                    f"No models found in registry.\n\n"
                    f"Configured paths:\n{paths_str}\n\n"
                    f"Would you like to scan these paths for models?",
                    confirm_variant="success",
                ),
                callback=self._on_initial_scan_prompt,
            )

    def _on_initial_scan_prompt(self, should_scan: bool) -> None:
        """Handle initial scan prompt response"""
        if should_scan:
            self._force_refresh_all()
        else:
            self.notify("Tip: Press 's' or '7' to open Settings and manage model paths", timeout=5)
