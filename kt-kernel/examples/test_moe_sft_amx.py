...
def test_moe_sft_forward(quant_mode: str = "bf16"):
    """
    ...
    print(f"\n--- Final Result ---")
    if diff < threshold:
        print(f"[OK] MOE SFT Forward Pass Test - {quant_mode.upper()} mode PASSED")
    else:
        print(f"[FAILED] MOE SFT Forward Pass Test - {quant_mode.upper()} mode FAILED")
...
def test_moe_sft_lora_weight_sync(quant_mode: str = "bf16"):
    ...
    """
    print(f"\n{'='*60}")
    print(f"Testing LoRA Weight Synchronization - {quant_mode.upper()} mode")
    print(f"{'='*60}")
    ...
    print(f"[OK] LoRA Weight Synchronization Test - {quant_mode.upper()} mode PASSED")


def test_moe_sft_training_loop(quant_mode: str = "bf16"):
    """
    ...
    """
    print(f"\n{'='*60}")
    print(f"Testing Complete Training Loop - {quant_mode.upper()} mode")
    print(f"{'='*60}")
    ...
    print(f"\n[OK] Training Loop Test - {quant_mode.upper()} mode PASSED")
...
def test_moe_sft_performance(quant_mode: str = "bf16"):
    """
    Test MOE SFT performance (forward + backward latency and throughput).
    ...
    print(f"\n{'='*60}")
    print(f"Performance Test - {quant_mode.upper()} mode")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"Performance Results - {quant_mode.upper()} mode")
    print(f"{'='*60}")
    print(f"\nForward Pass:")
...
def run_performance_tests():
    """Run performance tests for AMXBF16 and AMXINT8 modes."""
    print("\n" + "=" * 70)
    print(" MOE SFT AMX Performance Test Suite")
    print("=" * 70)
    print(f"Configuration:")
    ...
    try:
        for quant_mode in quant_modes:
            result = test_moe_sft_performance(quant_mode)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MOE SFT AMX Test Suite")
    parser.add_argument(
        "--mode",
