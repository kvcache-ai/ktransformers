...
bool run_optimizer_store_case(size_t count, bool accumulate, float scale) {
  std::mt19937 rng(7000 + static_cast<unsigned>(count) + static_cast<unsigned>(accumulate));
  std::vector<float> src(count);
  std::vector<ggml_bf16_t> dst(count);
  std::vector<float> expected(count);
  for (size_t i = 0; i < count; ++i) {
    src[i] = random_value(rng);
    dst[i] = GGML_FP32_TO_BF16(random_value(rng));
    expected[i] = src[i] * scale + (accumulate ? GGML_BF16_TO_FP32(dst[i]) : 0.0f);
  }

  avx::store_optimizer_gradient_bf16(dst.data(), src.data(), count, accumulate, scale);
  std::vector<float> actual(count);
  std::vector<float> rounded_expected(count);
  for (size_t i = 0; i < count; ++i) {
    actual[i] = GGML_BF16_TO_FP32(dst[i]);
    rounded_expected[i] = GGML_BF16_TO_FP32(GGML_FP32_TO_BF16(expected[i]));
  }

  char label[96];
  std::snprintf(label, sizeof(label), "optimizer_store_n%zu_acc%d_scale%.3f", count, accumulate ? 1 : 0, scale);
  return close(label, actual, rounded_expected);
}
...
int main() {
  ...
  ok &= run_case(7, 2048);
  ok &= run_case(42, 7168);
  for (size_t count : {size_t{1}, size_t{15}, size_t{16}, size_t{17}, size_t{31}, size_t{32}, size_t{47}}) {
    ok &= run_optimizer_store_case(count, false, 0.5f);
    ok &= run_optimizer_store_case(count, true, 0.25f);
  }
  return ok ? 0 : 1;
}
