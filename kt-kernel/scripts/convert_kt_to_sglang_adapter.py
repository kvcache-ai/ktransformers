...
KT_COMPOSITE_MANIFEST_FILE = "kt_composite_lora_manifest.json"
KT_COMPOSITE_MANIFEST_FORMAT = "sglang_kt_composite_lora_manifest"
...
def _write_composite_manifest(
    output_path: Path,
    *,
    expert_keys: Iterable[str],
    nonexpert_keys: Iterable[str],
    expert_targets: Iterable[str],
    nonexpert_targets: Iterable[str],
) -> None:
    manifest = {
        "format": KT_COMPOSITE_MANIFEST_FORMAT,
        "version": 1,
        "expert_keys": sorted(expert_keys),
        "nonexpert_keys": sorted(nonexpert_keys),
        "expert_target_modules": _ordered_target_modules(expert_targets),
        "nonexpert_target_modules": _ordered_target_modules(nonexpert_targets),
    }
    _write_json(output_path / KT_COMPOSITE_MANIFEST_FILE, manifest)
...
def convert_kt_to_sglang_adapter(
    ...
    *,
    ...
    if existing_tensors and fused_tensors:
        _write_composite_manifest(
            output_path,
            expert_keys=fused_tensors.keys(),
            nonexpert_keys=existing_tensors.keys(),
            expert_targets=fused_targets,
            nonexpert_targets=existing_targets,
        )
