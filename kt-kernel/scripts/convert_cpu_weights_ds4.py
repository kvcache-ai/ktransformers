...
def _dequantize_fp4_blockwise(
    ...
    nibbles = torch.stack([lo, hi], dim=-1).view(n, k)
    decoded = E2M1_VALUES.to(weight_u8.device)[nibbles]
    scale_fp32 = scale_bf16.to(torch.float32)
    scale_full = scale_fp32.repeat_interleave(group_size, dim=-1)
...
def load_model_config(input_path: str, input_type: str = None) -> Dict:
    ...
    Args:
        ...
        input_type: Input weight type (fp8/fp16/bf16/awq/fp4), used to validate quantization config
    ...
    model_type = text_cfg.get("model_type", config.get("model_type", ""))
    model_config["model_type"] = model_type

    if input_type == "fp4":
        expert_dtype = text_cfg.get("expert_dtype", "")
        if model_type == "deepseek_v4" and expert_dtype != "fp4":
            print(f"Warning: model_type is deepseek_v4 but expert_dtype='{expert_dtype}', expected 'fp4'")
        fp4_block_size = 32
        model_config["fp4_weight_block_size"] = fp4_block_size
        print(f"FP4 quantization config detected:")
        print(f"  model_type: {model_type}")
        print(f"  expert_dtype: {expert_dtype}")
        print(f"  fp4_weight_block_size: {fp4_block_size}")
    ...
    if input_type == "fp8":
        ...
        model_config["fp8_scale_fmt"] = quant_config.get("scale_fmt", "e4m3")
        print(f"FP8 quantization config detected:")
        print(f"  format: {quant_config.get('fmt', 'unknown')}")
        print(f"  scale_fmt: {quant_config.get('scale_fmt', 'unknown')}")
...
class ConverterBase:
    ...
    def __init__(
        ...
        # Auto-detect key format: HF vs V4 inference
        self.key_format = "hf"
        self._detect_key_format()
    ...
    def _detect_key_format(self):
        ...
        if any(".ffn.experts." in k for k in keys):
            self.key_format = "v4"
            if any(k.endswith(".w1.weight") for k in keys):
                print("Detected V4 inference key format: layers.{L}.ffn.experts.{E}.w1/w2/w3.{weight,scale}")
            else:
                print("Detected V4 key format (ffn.experts) but projection names unclear")
        elif any(".mlp.experts." in k for k in keys):
            self.key_format = "hf"
            print("Detected HF key format: model.layers.{L}.mlp.experts.{E}.{gate_up_proj,down_proj}.weight")
        else:
            print("Warning: could not detect expert key format")
    ...
    def _find_expert_layers(self) -> Dict[int, List[int]]:
        ...
        if self.key_format == "v4":
            # V4 inference format: layers.{L}.ffn.experts.{E}.w1.weight
            for key in self.tensor_file_map.keys():
                if ".ffn.experts." in key:
                    parts = key.split(".")
                    # layers.{L}.ffn.experts.{E}.w1.weight → parts: [layers, L, ffn, experts, E, w1, weight]
                    if len(parts) >= 5:
            ...
            print(f"Found {len(result)} layers with MoE experts (V4 format):")
    ...
    def convert(self, resume_layer: int = 0):
        ...
        if self.merge_to_safetensor:
            ...
            for key in self.tensor_file_map.keys():
                is_expert = ".mlp.experts." in key or ".ffn.experts." in key
                if not is_expert:
        ...
        if self.layout == "fused":
            ...
            def _expert_key(layer_idx, expert_id, proj_name, suffix):
                if self.key_format == "v4":
                    ...
                    v4_suffix = {"weight": "weight", "weight_scale_inv": "scale"}[suffix]
                    return f"layers.{layer_idx}.ffn.experts.{expert_id}.{v4_proj}.{v4_suffix}"
                else:
                    return f"model.layers.{layer_idx}.mlp.experts.{expert_id}.{proj_name}.{suffix}"
            ...
            for expert_id in expert_ids:
                for proj in proj_names:
                    w_key = _expert_key(layer_idx, expert_id, proj, "weight")
                    if w_key not in self.tensor_file_map:
                        raise KeyError(f"Missing {proj} weight for layer {layer_idx}, expert {expert_id} (key={w_key})")

                if self.input_type in ("fp8", "fp4"):
                    for proj in proj_names:
            ...
            if self.input_type == "fp8":
            ...
            elif self.input_type == "fp4":
                FP4_CHUNK = 64
                fp4_group_size = self.model_config.get("fp4_weight_block_size", 32)

                def _batch_dequant_fp4(proj_name):
                    from torch.profiler import record_function

                    fp4_list = []
                    scale_list = []
                    t_load = time.time()
                    with record_function(f"fp4_load_cpu_{proj_name}"):
                        for eid in expert_ids:
                            w = self._load_tensor(_expert_key(layer_idx, eid, proj_name, "weight"))
                            if w.dtype != torch.uint8:
                                w = w.view(torch.uint8)
                            fp4_list.append(w)
                            s = self._load_tensor(_expert_key(layer_idx, eid, proj_name, "weight_scale_inv"))
                            scale_list.append(s)
                    load_elapsed = time.time() - t_load
                    total_bytes = sum(t.nelement() * t.element_size() for t in fp4_list) + sum(
                        t.nelement() * t.element_size() for t in scale_list
                    )
                    speed_gbs = total_bytes / load_elapsed / 1e9 if load_elapsed > 0 else float("inf")
                    print(
                        f"    {proj_name}: loaded {len(fp4_list)} experts "
                        f"({total_bytes / 1e6:.1f} MB) in {load_elapsed:.3f}s "
                        f"= {speed_gbs:.2f} GB/s disk read"
                    )
                    ...
                    for i in range(0, len(fp4_list), FP4_CHUNK):
                        chunk_idx = i // FP4_CHUNK
                        with record_function(f"fp4_stack_{proj_name}_chunk{chunk_idx}"):
                            chunk_fp4 = torch.stack(fp4_list[i : i + FP4_CHUNK])
                            chunk_scale = torch.stack(scale_list[i : i + FP4_CHUNK])
                            C, N, K_packed = chunk_fp4.shape
                            _, _, K_div_gs = chunk_scale.shape

                        with record_function(f"fp4_dequant_{proj_name}_chunk{chunk_idx}"):
                            chunk_fp4_flat = chunk_fp4.reshape(C * N, K_packed).contiguous()
                            chunk_scale_flat = chunk_scale.reshape(C * N, K_div_gs).contiguous()
                            del chunk_fp4, chunk_scale

                            chunk_scale_bf16 = _ue8m0_to_bf16(chunk_scale_flat)

                            if torch.cuda.is_available():
                                chunk_fp4_flat = chunk_fp4_flat.cuda()
                                chunk_scale_bf16 = chunk_scale_bf16.cuda()

                            chunk_bf16 = _dequantize_fp4_blockwise(
                                chunk_fp4_flat, chunk_scale_bf16, group_size=fp4_group_size
                            )
                            del chunk_fp4_flat, chunk_scale_bf16

                            chunk_bf16_cpu = chunk_bf16.cpu()
                            del chunk_bf16

                        K = K_packed * 2
                        bf16_chunks.append(chunk_bf16_cpu.reshape(C, N, K))
                    dequant_elapsed = time.time() - t_dequant

                    with record_function(f"fp4_cat_{proj_name}"):
                        result = torch.cat(bf16_chunks, dim=0).contiguous()
                    print(f"    {proj_name}: fp4 dequant+transfer in {dequant_elapsed:.3f}s")
                    return result

                gate_proj = _batch_dequant_fp4("gate_proj")
                up_proj = _batch_dequant_fp4("up_proj")
                down_proj = _batch_dequant_fp4("down_proj")
                ...
                for expert_id in expert_ids:
                    gate_key = _expert_key(layer_idx, expert_id, "gate_proj", "weight")
                    up_key = _expert_key(layer_idx, expert_id, "up_proj", "weight")
                    down_key = _expert_key(layer_idx, expert_id, "down_proj", "weight")
...
def main():
    ...
    parser.add_argument(
        "--input-type",
        choices=["awq", "fp8", "fp16", "bf16", "fp4"],
        required=True,
        help="Input weight type (awq/fp8/fp16/bf16/fp4)",
    )
