...
public:
    ...
    void backward(const void* grad_output, ...);
...
# optimizer.step() 原地更新，下次 forward 自动看到
```
...
| AMX_SFT_MOE_TP 类 | operators/amx/sft_moe.hpp | 53-977 |
| forward_sft 实现 | operators/amx/sft_moe.hpp | 161-342 |
| backward 实现 | operators/amx/sft_moe.hpp | 358-396 |
| compute_lora_gate_up | operators/amx/sft_moe.hpp | 481-539 |
| compute_lora_down | operators/amx/sft_moe.hpp | 544-598 |
| TP_MOE_SFT 类 | operators/moe-sft-tp.hpp | 27-144 |
| Python 绑定 | ext_bindings.cpp | 749-781 |
| 测试代码 | examples/test_moe_sft_amx.py | 1-1372 |
