...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        final_hidden_states = torch.zeros(
            (batch_size * sequence_length, hidden_dim), dtype=hidden_states.dtype, device="cpu"
        )

        selected_experts = selected_experts.to("cpu")
        routing_weights = routing_weights.to("cpu")
        ...
        final_hidden_states = final_hidden_states.to(hidden_states.device)
...
class Qwen2MoeForCausalLM(Qwen2MoePreTrainedModel):
    ...
    def place_weights(self):
        ...
        for i in range(len(self.model.layers)):
            ...
            self.model.layers[i].mlp.shared_expert.to("cuda:0")
            self.model.layers[i].mlp.shared_expert_gate.to("cuda:0")
