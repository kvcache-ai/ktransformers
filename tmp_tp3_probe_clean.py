...
LOG = Path("/mnt/data2/work/tp3_probe_clean.log")
SAMPLES = Path("/mnt/data2/work/tp3_probe_clean_samples.jsonl")
PORT = 31675
...
for cmd in [
    "pkill -KILL -f 'sglang.launch_server.*31675|kt run /mnt/data2/models/Qwen3.5-35B-A3B --port 31675'",
]:
...
cmd = [
    ...
    "baseline",
    "--served-model-name",
    "Qwen3.5-35B-A3B-16g-wave-probe-tp3",
    ...
    if scope is None and LOG.exists():
        ...
        if m:
            ...
            cg = subprocess.check_output(
                ["systemctl", "--user", "show", "-p", "ControlGroup", "--value", scope], text=True
            ).strip()
...
print(json.dumps({"pid": proc.pid, "scope": scope, "status": status, "log": str(LOG), "samples": str(SAMPLES)}))
